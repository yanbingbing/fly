...
class Image
{
    ...
    public function __construct($filename = null, array $options = array(), $binary = false, $type = null)
    {
        ...
        if (!$this->oldImage) {
            ...
            throw new Exception\InvalidArgumentException('create image with given resource failed' . ($error ? (' with error ' . $error['message']) : ''));
        }
    ...
    public function resize($maxWidth, $maxHeight)
    {
        ...
        if (!is_numeric($maxWidth)) {
            throw new Exception\InvalidArgumentException('$maxWidth must be numeric');
        }

        if (!is_numeric($maxHeight)) {
            throw new Exception\InvalidArgumentException('$maxHeight must be numeric');
        }
    ...
    public function resizeCrop($width, $height)
    {
        if (!is_numeric($width) || $width == 0) {
            throw new Exception\InvalidArgumentException('$width must be numeric and greater than zero');
        }

        if (!is_numeric($height) || $height == 0) {
            throw new Exception\InvalidArgumentException('$height must be numeric and greater than zero');
        }
    ...
    public function resizePercent($percent = 0)
    {
        if (!is_numeric($percent)) {
            throw new Exception\InvalidArgumentException ('$percent must be numeric');
        }
    ...
    public function crop($startX, $startY, $cropWidth, $cropHeight)
    {
        ...
        if (!is_numeric($startX)) {
            throw new Exception\InvalidArgumentException('$startX must be numeric');
        }

        if (!is_numeric($startY)) {
            throw new Exception\InvalidArgumentException('$startY must be numeric');
        }

        if (!is_numeric($cropWidth)) {
            throw new Exception\InvalidArgumentException('$cropWidth must be numeric');
        }

        if (!is_numeric($cropHeight)) {
            throw new Exception\InvalidArgumentException('$cropHeight must be numeric');
        }
    ...
    public function rotateImageNDegrees($degrees)
    {
        if (!is_numeric($degrees)) {
            throw new Exception\InvalidArgumentException('$degrees must be numeric');
        }

        if (!function_exists('imagerotate')) {
            throw new Exception\RuntimeException('Your version of GD does not support image rotation.');
        }
    ...
    public function show($send_header = true, $format = null)
    {
        if ($send_header && headers_sent()) {
            throw new Exception\RuntimeException('headers have already been sent');
        }

        $format = ($format !== null) ? strtolower($format) : $this->format;

        if (!in_array($format, self::$allowedFormats)) {
            throw new Exception\InvalidArgumentException ('Invalid format type specified in save function: ' . $format);
        }
    ...
    public function save($filename, $format = null)
    {
        ...
        if (!is_writeable(dirname($filename))) {
            ...
            if ($this->options['correctPermissions'] === true) {
                ...
                if (!is_writeable(dirname($filename))) {
                    throw new Exception\RuntimeException ('File is not writeable, and could not correct permissions: ' . $filename);
                }
            } // throw an exception if not writeable
            else {
                throw new Exception\RuntimeException ('File not writeable: ' . $filename);
            }
        }
    ...
    protected function assertFileAvaiable()
    {
        ...
        if (!is_file($this->filename)) {
            throw new Exception\InvalidArgumentException('Image file not exists: ' . $this->filename);
        }

        if (!is_readable($this->filename)) {
            throw new Exception\RuntimeException('Image file not readable: ' . $this->filename);
        }
    }
    ...
    protected function detectFormat($type = null)
    {
        if ($this->binary) {
            if (!$type || !in_array($type, self::$allowedFormats)) {
                throw new Exception\InvalidArgumentException('Binary image type must be specified');
            }
            $mime = 'image/' . $type;
        }
        ...
        if (empty($mime)) {
            ...
            if ($info === false) {
                throw new Exception\RuntimeException('Read image infomation failed: ' . $this->filename);
            }
        ...
        switch ($mime) {
            ...
            case 'image/gif':
                ...
                break;
            default:
                throw new Exception\RuntimeException('Image format not supported: ' . $mime);
                break;
        }

        if (!$compatible) {
            throw new Exception\RuntimeException('Your GD installation does not support ' . $this->format . ' image types');
        }
    }
